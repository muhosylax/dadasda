...
if (!member) return message.channel.send(embed.setDescription(`${message.author} Lütfen bir kullanıcı belirtin @dsng/İD gibi.`)).then(tedoa => tedoa.delete({timeout : 8000})).then(message.react(config.carpi))
