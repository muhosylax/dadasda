...
let lowzy = message.guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== message.guild.id).size == 0)
...
.setColor("#000069")
.setDescription("Sunucuda rolü olmayan \`"+ lowzy.size +"\` kişiye kayıtsız rolü verildi!")
message.channel.send(lore).then(m => message.react(config.onay))
} else if(!args[0]) {
const cxyber = new Discord.MessageEmbed()
.setAuthor(""+message.author.username +" ", message.author.avatarURL())
.setColor("#000069")
.setDescription("Sunucumuzda rolü olmayan \`"+ lowzy.size +"\` kişi var. Bu kişilere kayıtsız rolü vermek için \`.rolsüz ver\` komutunu uygulayın!")
